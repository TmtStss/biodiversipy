...
def tif_to_df(file_path, plot=False, coords=False, column_name="val"):
    ...
    if coords:
        ...
        subset_lon = np.logical_and(df.columns >= coords["lon_lower"], df.columns <= coords["lon_upper"])

        subset_lat = np.logical_and(df.index >= coords["lat_lower"], df.index <= coords["lat_upper"])
    ...
    if plot:
        ...
        image = ax.imshow(df_masked, extent=[x_min, x_max, y_min, y_max])
...
def clean_occurences(csv, n=0):
    ...
    data_cleaned = data_cleaned.drop_duplicates(
        subset=["decimalLatitude", "decimalLongitude", "taxonKey"], keep="first"
    )
...
def append_features(occurences_path, features_path, from_csv=True):
    ...
    df = occurences.conditional_join(
        features,
        ("latitude", "lat_lower", ">="),
        ("latitude", "lat_upper", "<"),
        ("longitude", "lon_lower", ">="),
